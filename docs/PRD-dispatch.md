...
- F2.2: System instruction: a **spec-shaping skill** — a named, versioned instruction block in `server/anthropic/spec-skills.ts`, prepended to the injected repo context. It drives toward the same artifact the original one-paragraph instruction named (one-line title; problem statement; acceptance criteria as a checklist; likely files/modules affected; test plan; out-of-scope notes) and keeps the one-question-per-turn rule, but adds the method for getting there from a vague idea: act as a thinking partner rather than a requirements extractor, ground claims in real files via the chat's `read_file`/`list_files` tools, and probe the rigor gaps (evidence, specificity, counterfactual, attachment) that are actually present. Adapted from `ce-brainstorm` (EveryInc/compound-engineering-plugin, MIT). The registry is keyed by skill id so additional modes are additive; one is registered today.
